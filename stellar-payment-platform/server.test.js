...
describe("rejectNestedObjects middleware", () => {
  ...
  test("passes through when body contains only string values", () => {
    rejectNestedObjects(
      { query: {}, body: { username: "alice*localhost", address: "GABC123" } },
      res,
      next,
    );
  ...
  test("passes through when body is undefined (no-body GET requests)", () => {
    rejectNestedObjects(
      { query: { address: "GABC123" }, body: undefined },
      res,
      next,
    );
    expect(next).toHaveBeenCalledTimes(1);
      ...
      all: jest.fn((sql, params, cb) => {
        ...
        const rows = [
          ...
          },
          {
            ...
            address: "GBOB0000000000000000000000000000000000000000000000000000",
...
describe("GET /federation — memo fields in response", () => {
  ...
  test("omits memo fields when user has no memo configured", async () => {
    ...
    const res = await request(app).get(
      "/federation?q=alice*localhost&type=name",
    );
    expect(res.status).toBe(200);
    expect(res.body).not.toHaveProperty("memo_type");
    expect(res.body).not.toHaveProperty("memo");
  ...
  test("returns stored id memo in type=id federation response", async () => {
    ...
    const res = await request(app).get(
      `/federation?q=${VALID_ADDRESS}&type=id`,
    );
    expect(res.status).toBe(200);
...
describe("API v1 routing", () => {
  ...
  test("POST /api/v1/register/bulk returns 400 when body is not an array", async () => {
    const res = await request(app)
      .post("/api/v1/register/bulk")
      .send({ username: "alice", address: "GABC" });

    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/Request body must be an array/);
  });

  test("POST /api/v1/register/bulk returns 400 when too many records are provided", async () => {
    const records = Array.from({ length: 101 }, (_, i) => ({
      username: `user${i}`,
      address: `G${"A".repeat(55)}${i}`,
    }));

    const res = await request(app).post("/api/v1/register/bulk").send(records);

    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/Maximum bulk registration size/);
  });

  test("POST /api/v1/register/bulk returns per-item success and errors", async () => {
    prisma.user.findMany.mockResolvedValue([
      { username: "existing*localhost", address: "GEXISTINGADDRESS" },
    ]);
    ...
    const res = await request(app)
      ...
      .send([
        {
          username: "alice",
          address: "GVALIDADDRESS0000000000000000000000000000000000000",
        },
        { username: "existing", address: "GEXISTINGADDRESS" },
        { username: "bob", address: "SINVALIDSECRET" },
      ]);
    ...
    expect(res.body.results).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          address: "GEXISTINGADDRESS",
          success: false,
          error: expect.stringContaining("already registered"),
        }),
        expect.objectContaining({
          username: "bob*localhost",
          address: "SINVALIDSECRET",
          success: false,
          error: expect.stringContaining("Secret Key"),
        }),
      ]),
    );
  });
