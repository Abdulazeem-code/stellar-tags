...
const getLocalUserByAddress = async (address) =>
  poolGet(
    "SELECT username, address FROM username_registry WHERE address = ? AND deleted_at IS NULL LIMIT 1",
    [address],
  );

const getLocalUserByUsername = async (username) =>
  poolGet(
    "SELECT username, address FROM username_registry WHERE username = ? AND deleted_at IS NULL LIMIT 1",
    [username],
  );
...
app.post("/register", async (req, res, next) => {
  ...
  if (profanityFilter.isProfane(usernameLocalPart)) {
    return res
      .status(400)
      .json({ error: "Username contains restricted words" });
  }
...
app.get("/.well-known/stellar.toml", (_req, res) => {
  ...
  res.send(
    'FEDERATION_SERVER="https://stellar-tags-production.up.railway.app/federation"\n',
  );
});
...
app.use((err, _req, res, _next) => {
  ...
  if (statusCode === 500) {
    ...
    console.error(
      `[Correlation ID: ${req.correlationId}] [Error ID: ${errorId}]`,
      err,
    );
